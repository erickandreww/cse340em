...
async function getVehicleByInventoryId(inv_id) {
  try {
    const data = await pool.query(
      `SELECT * FROM public.inventory WHERE public.inventory.inv_id = $1`,
      [inv_id]
    ) 
    return data.rows[0]
  } catch (error) {
...
async function registerNewVehicle(inv_make, inv_model, inv_year, inv_description, inv_image, 
 ...
 * ************************** */
async function updateInventory( inv_id, inv_make, inv_model,
  inv_description, inv_image, inv_thumbnail, inv_price, inv_year,
